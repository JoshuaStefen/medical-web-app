...
contract DoctorRegistration {
    ...
    string[] private doctorKeys; // Store doctor keys for iteration
    ...
    function registerDoctor(
        ...
        doctorKeys.push(_hhNumber); // Store doctor key
    ...
    function grantPermission(
        ...
        require(doctorAddresses[_doctorNumber] != address(0), "Doctor not registered");
    ...
    function getDoctorList() public view returns (Doctor[] memory) {
        Doctor[] memory doctorArray = new Doctor[](doctorKeys.length);
        
        for (uint i = 0; i < doctorKeys.length; i++) {
            doctorArray[i] = doctors[doctorKeys[i]];
        }
