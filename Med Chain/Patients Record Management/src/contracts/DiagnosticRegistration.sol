...
contract DiagnosticRegistration {
    ...
    string[] private diagnosticKeys; // Track registered diagnostics
    ...
    function registerDiagnostic(
        ...
        diagnosticKeys.push(_hhNumber); // Store diagnostic key
    ...
    function getDiagnosticList() public view returns (Diagnostic[] memory) {
        Diagnostic[] memory diagnosticArray = new Diagnostic[](diagnosticKeys.length);
        
        for (uint i = 0; i < diagnosticKeys.length; i++) {
            diagnosticArray[i] = diagnostics[diagnosticKeys[i]];
        }
